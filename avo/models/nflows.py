...
class IAF(nn.Module):
    def __init__(self, in_dim, h_dim, depth, activation="ReLU", num_stable=False):
        ...
        self.IAFs = nn.ModuleList([
            IATransform(in_dim=in_dim, h_dim=h_dim, rand_perm=True, activation=activation, num_stable=num_stable) for i
            in range(depth)
        ])
...
class Householder(nn.Module):
    ...
    def forward(self, z, v=None):
        ...
        # v * v_T
        vvT = (v * v).sum(
            1)  # memory inefficient, however fast, could use author instead torch.bmm( v.unsqueeze(2), v.unsqueeze(
        # 1) )  # v * v_T : batch_dot( B x L x 1 * B x 1 x L ) = B x L x L
        # v * v_T * z
        vvTz = (vvT * z).sum(
            1)  # torch.bmm( vvT, z.unsqueeze(2) ).squeeze(2) # A * z : batchdot( B x L x L * B x L x 1 ).squeeze(2)
        # = (B x L x 1).squeeze(2) = B x L
        # calculate norm ||v||^2
